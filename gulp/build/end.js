...
gulp.task('build-end', ['sourcemap-fix'], function () {
  ...
  return gulp.src(paths.bundles.css.app, {read: false})
    .pipe(rimraf());
});
...
gulp.task('sourcemap-fix', function () {
  ...
  return fs.readFile(paths.bundles.css.map, 'utf8', function (err, data) {
    if (err) return utils.logError(err);
    var res = data.replace(new RegExp(pathToRemove, 'g'), '');

    fs.writeFile(paths.bundles.css.map, res, 'utf8', function (err) {
      if (err) return utils.logError(err);
    });
  });
});
